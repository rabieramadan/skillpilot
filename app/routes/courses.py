...
def update_material_progress(course_id, material_id):
    ...
    try:
        ...
        # When a material is completed, refresh the learner's adaptive
        # state so the "Adaptive progress" widget reflects the new mastery
        # before the next page load. Best-effort, never blocks the response.
        if progress.status == 'completed':
            try:
                from app.services.recommendation_engine import (
                    trigger_adaptive_recompute,
                )
                trigger_adaptive_recompute(user_id, course_id, source='material')
            except Exception:
                pass
