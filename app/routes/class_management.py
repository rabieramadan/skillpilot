    ...
    try:
        ...
        for r in results:
            ...
            student_results.append({
                ...
                'force_submitted': violation_count >= 3,
            })
...
@class_mgmt_bp.route('/classes/<course_id>/exams/<exam_id>/results/export-csv', methods=['GET'])
@class_access_required
def export_exam_results_csv(course_id, exam_id):
    """Export exam results as CSV (kept for backwards compat — prefer xlsx)."""
    import csv, io
    db = get_db()
    if not db:
        return jsonify({'error': 'Database not configured'}), 500
    try:
        ...
        if not exam:
            return jsonify({'error': 'Exam not found'}), 404
        results = ExamResult.query.filter_by(exam_id=exam_id).order_by(ExamResult.submitted_at).all()
        ...
        writer.writerow(['Student Name', 'Username', 'Attempt #', 'Score', 'Total Points',
                         'Percentage %', 'Passed', 'Time Spent (s)', 'Violations',
                         'Violation Types', 'Auto-Submitted', 'Submitted At'])
        for r in results:
            ...
            for ev in events:
                ...
                event_counts[t] = event_counts.get(t, 0) + 1
            violation_count = getattr(r, 'violation_count', 0) or 0
            event_summary = '; '.join(f"{k}×{v}" for k, v in event_counts.items() if k not in ('tab_focus', 'contextmenu_blocked'))
            writer.writerow([
                _csv_safe(student.full_name if student else 'Unknown'),
                _csv_safe(student.username if student else ''),
                r.attempt_number, r.score, r.total_points,
                round(r.percentage or 0, 1),
                'Yes' if r.passed else 'No',
                r.time_spent_seconds or 0, violation_count,
                _csv_safe(event_summary),
                'Yes' if violation_count >= 3 else 'No',
                r.submitted_at.strftime('%Y-%m-%d %H:%M') if r.submitted_at else '',
            ])
        safe_title = ''.join(c for c in (exam.title or 'exam') if c.isalnum() or c in (' ', '-', '_'))[:40]
        from flask import Response
        return Response(buf.getvalue(), mimetype='text/csv',
                        headers={'Content-Disposition': f'attachment; filename="{safe_title}_results.csv"'})
    except Exception as e:
        print(f"Export exam results CSV error: {e}")
        return jsonify({'error': str(e)}), 500


@class_mgmt_bp.route('/classes/<course_id>/exams/<exam_id>/results/export-xlsx', methods=['GET'])
@class_access_required
def export_exam_results_xlsx(course_id, exam_id):
    """Export exam results + integrity warnings as a formatted Excel workbook."""
    import io
    db = get_db()
    if not db:
        return jsonify({'error': 'Database not configured'}), 500
    try:
        ...
        from app.models import Exam, ExamResult, User, Course
        ...
        PROCTOR_LABELS = {
            'tab_blur': 'Left window', 'paste_detected': 'Paste attempt',
            'copy_detected': 'Copy attempt', 'fullscreen_exit': 'Exited fullscreen',
        ...
        red_hdr = PatternFill('solid', fgColor='C62828')
        green_hdr = PatternFill('solid', fgColor='166534')
        row_red    = PatternFill('solid', fgColor='FFF0F0')
        row_orange = PatternFill('solid', fgColor='FFF8EE')
        row_green  = PatternFill('solid', fgColor='F0FDF4')
        row_plain  = PatternFill('solid', fgColor='FAFAFA')
        ...
        ws['A2'] = f'Course: {course.title if course else ""}   |   Generated: {__import__("datetime").datetime.now().strftime("%Y-%m-%d %H:%M")}'
        ws['A2'].font = Font(italic=True, size=10, color='555555')
        ws['A2'].alignment = center

        # ── Column headers ──────────────────────────────────────────
        headers = [
            ('Student Name', 22), ('Username', 16), ('Attempt #', 10),
            ('Score', 9), ('Total Pts', 9), ('Percentage', 11),
            ('Result', 9), ('Time (s)', 9),
            ('Violations', 11), ('Violation Details', 30),
            ('Auto-Submitted', 14), ('Submitted At', 18),
        ]
        ...
        for r in results:
            ...
            for ev in events:
                t = (ev or {}).get('type', 'unknown')
                event_counts[t] = event_counts.get(t, 0) + 1
            vc = getattr(r, 'violation_count', 0) or 0
            auto = vc >= 3
            event_summary = '; '.join(
                f"{PROCTOR_LABELS.get(k, k)} ×{v}"
                for k, v in event_counts.items()
                if k not in ('tab_focus', 'contextmenu_blocked')
            )
            if vc > 0: flagged_count += 1
            if auto: auto_count += 1

            fill = row_red if auto else (row_orange if vc > 0 else (row_green if r.passed else row_plain))
            row_data = [
                ...
                vc,
                ('AUTO-SUBMITTED — ' if auto else '') + event_summary if vc > 0 else 'Clean',
                'Yes' if auto else 'No',
                r.submitted_at.strftime('%Y-%m-%d %H:%M') if r.submitted_at else '',
            ]
        ...
        ws.cell(row=sum_row, column=1, value=f'Total: {total} submissions | Passed: {passed} | Failed: {total-passed}').font = bold
        ws.cell(row=sum_row, column=1).alignment = left

        ws.merge_cells(f'D{sum_row}:L{sum_row}')
        integrity_text = (
            f'⚠ {flagged_count} submission(s) have integrity flags. '
            f'{auto_count} auto-submitted at 3 violations.'
        ) if flagged_count else '✓ No integrity issues detected.'
        summary_cell = ws.cell(row=sum_row, column=4, value=integrity_text)
        summary_cell.font = Font(bold=True, color='C62828' if flagged_count else '166534')
        ...
        print(f"Export exam results XLSX error: {e}")
        return jsonify({'error': str(e)}), 500
...
def download_certificate(course_id, enrollment_id):
    ...
    try:
        ...
        # Generate QR code
        import os as _os
        _verify_base = _os.environ.get('CERTIFICATE_VERIFY_BASE_URL', '').rstrip('/')
        if not _verify_base:
            try:
                _verify_base = request.host_url.rstrip('/')
            except Exception:
                _verify_base = 'https://futurecoverage.ai'
        verification_url = f"{_verify_base}/verify/{cert_number}"
...
# ============================================
# CLASS LIST FOR TEACHERS / ADMINS
# ============================================

@class_mgmt_bp.route('/classes/list', methods=['GET'])
def list_classes_for_teacher():
    """Return courses accessible to the current user (teacher sees own classes, admin sees all)."""
    db = get_db()
    if not db:
        return jsonify({'error': 'Database not configured'}), 500
    user_id = session.get('user_id')
    ...
    try:
        ...
        if user_role in ['super_admin', 'superadmin', 'admin', 'institution_admin']:
            courses = Course.query.order_by(Course.title).all()
        elif user_role in ['instructor', 'teacher']:
            assigned = CourseInstructor.query.filter_by(user_id=user_id).all()
        ...
        for c in courses:
            classes.append({
                ...
                'capacity': getattr(c, 'capacity', 30) or 30,
        ...
        print(f"list_classes_for_teacher error: {e}")
        return jsonify({'error': str(e)}), 500


# ============================================
# REGISTRATION TOGGLE — teacher/admin accessible
# ============================================

@class_mgmt_bp.route('/classes/<course_id>/registration', methods=['PATCH'])
@class_access_required
def toggle_class_registration(course_id):
    """Open or close registration for a course. Accessible to teachers assigned to the course."""
    db = get_db()
    if not db:
        return jsonify({'error': 'Database not configured'}), 500
    try:
        ...
        return jsonify({
            ...
            'message': f'Registration is now {state} for "{course.title}"'
        })
    except Exception as e:
        db.session.rollback()
        print(f"toggle_class_registration error: {e}")
        return jsonify({'error': str(e)}), 500
