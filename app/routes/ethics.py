...
ethics_pages_bp = Blueprint('ethics_pages', __name__)


@ethics_pages_bp.route('/ethicalsense', methods=['GET'])
def ethicalsense_hub():
    if 'user_id' not in session:
        from flask import redirect
        return redirect('/skillpilot-landing')
    return render_template('ethicalsense_hub.html')


@ethics_pages_bp.route('/admin/ethics-settings', methods=['GET'])
def admin_ethics_settings_page():
    from flask import redirect
    if 'user_id' not in session:
        return redirect('/skillpilot-landing')
    role = (session.get('role') or '').lower()
    if not (session.get('is_admin') or role in ('admin', 'superadmin', 'super_admin')):
        return redirect('/app')
    return render_template('admin_ethics_settings.html')

# Data file paths
ETHICS_DATA_FILE = 'ethics_assessments.json'
ETHICS_SETTINGS_FILE = 'ethics_settings.json'

# Global AI ethics principles — the international convergence framework
# (transparency, fairness, non-maleficence, responsibility, privacy).
# Every generated scenario, every score breakdown, and every certificate
# is anchored to these five principles.
AI_ETHICS_PRINCIPLES = [
    {
        'id': 'transparency',
        'name_en': 'Transparency',
        'name_ar': 'الشفافية',
        'desc_en': 'Decisions, data sources, and AI behavior are explainable and disclosed to those affected.',
        'desc_ar': 'القرارات ومصادر البيانات وسلوك الذكاء الاصطناعي قابلة للتفسير ومُفصح عنها للمتأثرين.',
        'icon': 'fa-eye',
    },
    {
        'id': 'fairness',
        'name_en': 'Fairness',
        'name_ar': 'العدالة',
        'desc_en': 'Outcomes avoid unjust bias and treat individuals and groups equitably.',
        'desc_ar': 'النتائج تتجنّب التحيّز غير العادل وتعامل الأفراد والمجموعات بإنصاف.',
    ...
    },
    {
        ...
        'name_ar': 'عدم الإضرار',
        'desc_en': 'Systems are designed and operated to prevent foreseeable harm to people, society, and the environment.',
        'desc_ar': 'تُصمّم الأنظمة وتُشغَّل لمنع الأذى المتوقّع للأفراد والمجتمع والبيئة.',
    ...
    },
    {
        ...
        'name_ar': 'المسؤولية',
        'desc_en': 'Clear accountability is assigned for outcomes, with mechanisms for redress and human oversight.',
        'desc_ar': 'تُحدَّد المساءلة الواضحة للنتائج مع آليات للتظلّم والإشراف البشري.',
        'icon': 'fa-handshake',
    },
    {
        'id': 'privacy',
        'name_en': 'Privacy',
        'name_ar': 'الخصوصية',
        'desc_en': 'Personal data is collected lawfully, minimized, protected, and used only for stated purposes.',
        'desc_ar': 'تُجمع البيانات الشخصية وفق القانون وبأقل قدر، وتُحمى وتُستخدم لأغراضها المعلنة فقط.',
        'icon': 'fa-user-shield',
    },
]


def get_questions_for_level(level_id: str, settings: dict | None = None, fallback: int = 20) -> int:
    """Return the admin-configured question count for a level, enforcing a minimum of 20."""
    try:
        data = load_ethics_data()
        admin_settings = data.get('admin_settings', {}) or {}
        qpl = admin_settings.get('questions_per_level', {}) or {}
        # level_id may be like 'basic' or 'general-basic' — normalize to suffix
        key = level_id.split('-')[-1] if '-' in str(level_id) else str(level_id)
        if key in qpl:
            try:
                return max(20, int(qpl[key]))
            except (TypeError, ValueError):
                pass
    except Exception:
        pass
    return max(20, int(fallback or 20))


def get_principle_for_question(question_number: int) -> dict:
    """Pick a principle for a given question number, rotating through the five."""
    idx = (max(1, int(question_number)) - 1) % len(AI_ETHICS_PRINCIPLES)
    return AI_ETHICS_PRINCIPLES[idx]
    ...
    if not os.path.exists(ETHICS_DATA_FILE):
        return {
            ...
            'admin_settings': {
                ...
                'free_levels': {'basic': False, 'intermediate': False, 'advanced': False},
                'questions_per_level': {'basic': 20, 'intermediate': 20, 'advanced': 20},
                'waived_users': [],
                'passing_score': 70,
                'paypal_payment_link': ''
            }
...
def update_admin_settings():
    ...
    # Enforce min 20 questions per level on the server side too
    if isinstance(new_settings.get('questions_per_level'), dict):
        cleaned = {}
        for k, v in new_settings['questions_per_level'].items():
            try:
                cleaned[k] = max(20, int(v))
            except (TypeError, ValueError):
                cleaned[k] = 20
        new_settings['questions_per_level'] = cleaned
...
@ethics_bp.route('/principles', methods=['GET'])
def get_principles():
    """Return the five global AI ethics principles used to anchor every assessment."""
    return jsonify({'success': True, 'principles': AI_ETHICS_PRINCIPLES})


@ethics_bp.route('/level-summary', methods=['GET'])
def level_summary():
    """Lightweight summary of the three levels used by the EthicalSense hub.
    Returns admin-configured question count, price, and free flag per level."""
    data = load_ethics_data()
    admin_settings = data.get('admin_settings', {}) or {}
    pricing = admin_settings.get('pricing', {}) or {}
    free_levels = admin_settings.get('free_levels', {}) or {}
    qpl = admin_settings.get('questions_per_level', {}) or {}
    passing_score = int(admin_settings.get('passing_score', 70) or 70)
    out = []
    for lid in ('basic', 'intermediate', 'advanced'):
        try:
            q = max(20, int(qpl.get(lid, 20)))
        except (TypeError, ValueError):
            q = 20
        try:
            price = float(pricing.get(lid, 0) or 0)
        except (TypeError, ValueError):
            price = 0.0
        is_free = bool(free_levels.get(lid)) or price <= 0
        out.append({
            'id': lid,
            'questions': q,
            'price': price,
    ...
    return jsonify({'success': True, 'levels': out, 'passing_score': passing_score})


@ethics_bp.route('/me-context', methods=['GET'])
@authenticated_required
def me_context():
    """Return profile fields the EthicalSense hub uses to prefill the assessment intake."""
    user_id = session.get('user_id')
    try:
        from app.models import User, LearnerProfile
        u = User.query.filter_by(id=user_id).first()
        lp = LearnerProfile.query.filter_by(user_id=user_id).first() if u else None
    except Exception:
        u, lp = None, None
    return jsonify({
        'success': True,
        'context': {
            'jobTitle': (getattr(u, 'job_title', None) if u else None) or (getattr(lp, 'target_role', None) if lp else None) or '',
            'profession': (getattr(u, 'profession', None) if u else None) or (getattr(lp, 'primary_goal', None) if lp else None) or '',
            'country': (getattr(u, 'country', None) if u else None) or (getattr(u, 'region', None) if u else None) or '',
            'gender': (getattr(u, 'gender', None) if u else None) or '',
            'educationLevel': (getattr(u, 'education_level', None) if u else None) or (getattr(u, 'education', None) if u else None) or '',
            'language': (getattr(u, 'language', None) if u else None) or (getattr(lp, 'preferred_language', None) if lp else None) or 'en',
            'domain': (getattr(lp, 'target_role', None) if lp else None) or (getattr(u, 'profession', None) if u else None) or 'general',
            'name': (getattr(u, 'full_name', None) if u else None) or (getattr(u, 'username', None) if u else None) or '',
            'email': (getattr(u, 'email', None) if u else None) or '',
        }
    })


@ethics_bp.route('/admin/users-search', methods=['GET'])
@authenticated_required
@admin_required
def admin_users_search():
    """Search users by email or name to manage per-user payment waivers."""
    ...
    try:
        ...
        users = []
    data = load_ethics_data()
    waived = set(data.get('admin_settings', {}).get('waived_users', []) or [])
    for u in users:
        u['waived'] = u['id'] in waived
...
def check_payment_status():
    ...
    # Admin override: per-level free flag (basic|intermediate|advanced) or by level_id
    free_levels = admin_settings.get('free_levels', {}) or {}
    pricing_override = admin_settings.get('pricing', {}) or {}
    level_key = (level_info or {}).get('difficulty') or level_id
    if free_levels.get(level_key) or free_levels.get(level_id):
        amount = 0
    elif level_key in pricing_override:
        try:
            amount = float(pricing_override[level_key])
        except (TypeError, ValueError):
            pass

    is_free_level = (amount is not None and float(amount) <= 0)

    # No payment required if waived OR verified OR level is free OR amount is 0
    payment_required = not (payment_waived or verified_payment or is_free_level)
...
def create_ethics_session():
    ...
    # Auto-prefill missing personalization fields from the user's stored profile
    # so the user is not asked to re-enter what they already provided.
    try:
        from app.models import User, LearnerProfile
        u = User.query.filter_by(id=user_id).first()
        lp = LearnerProfile.query.filter_by(user_id=user_id).first() if u else None
        prefill = {
            'jobTitle': (getattr(u, 'job_title', None) or getattr(lp, 'target_role', None) or '') if (u or lp) else '',
            'profession': (getattr(u, 'profession', None) or getattr(lp, 'primary_goal', None) or '') if (u or lp) else '',
            'country': (getattr(u, 'country', None) or getattr(u, 'region', None) or '') if u else '',
            'gender': (getattr(u, 'gender', None) or '') if u else '',
            'educationLevel': (getattr(u, 'education_level', None) or getattr(u, 'education', None) or '') if u else '',
            'language': (getattr(u, 'language', None) or getattr(lp, 'preferred_language', None) or 'en') if (u or lp) else 'en',
            'domain': (getattr(lp, 'target_role', None) or getattr(u, 'profession', None) or 'general') if (u or lp) else 'general',
        }
        for k, v in prefill.items():
            if v and not req_data.get(k):
                req_data[k] = v
    except Exception:
        pass

    required_fields = ['level', 'domain', 'jobTitle', 'profession', 'country', 'gender', 'educationLevel', 'language']
    missing = [f for f in required_fields if not req_data.get(f)]

    if missing:
        return jsonify({'error': f'Missing fields: {", ".join(missing)}', 'missing_fields': missing}), 400
    ...
    new_session = {
        ...
        'total_questions': get_questions_for_level(req_data['level'], settings, fallback=level_info.get('questions', 20)),
        'current_question': 0,
...
Write the entire scenario, options, and theme in Arabic script (العربية)."""
        ...
        # Anchor every scenario to one of the five global AI ethics principles
        # (transparency, fairness, non-maleficence, responsibility, privacy).
        principle = get_principle_for_question(question_number)
        principle_name = principle['name_ar'] if language_code == 'ar' else principle['name_en']
        principle_desc = principle['desc_ar'] if language_code == 'ar' else principle['desc_en']

        prompt = f"""Generate a professional ethics scenario for assessment purposes.

This scenario MUST test the candidate's judgment on the global AI ethics principle of:
- Principle: {principle['id']} ({principle_name})
- Definition: {principle_desc}
...
Create a realistic ethical dilemma where this specific principle is at stake, with 4 possible actions.
The "correct_option" must be the one that BEST upholds the named principle.
Each option should be plausible — at least one should be a tempting but wrong choice that violates the principle.
Include a short "rationale" explaining why the correct option upholds the principle and why the others fall short.
...
{{
    ...
    "theme": "ethical theme",
    "rationale": "1-2 sentence explanation of the correct answer in the same language",
...
Only respond with valid JSON, no additional text."""
        ...
        # Save scenario (anchored to one of the five global AI ethics principles)
        scenario_record = {
            'session_id': session_id,
            ...
            'principle': scenario_data.get('principle', principle['id']),
            'principle_name': scenario_data.get('principle_name', principle_name),
        ...
        return jsonify({
            ...
            'scenario': {
                ...
                'options': scenario_data['options'],
                'question_number': question_number,
                'principle': scenario_record['principle'],
                'principle_name': scenario_record['principle_name']
            }
        })
