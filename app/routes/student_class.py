...
def start_class_exam(course_id, exam_id):
    ...
    try:
        ...
        max_allowed = exam.max_attempts if (exam.max_attempts and exam.max_attempts > 0) else 1
        if attempts_used >= max_allowed:
            return jsonify({'error': f'Maximum attempts reached ({max_allowed}). You have used all {attempts_used} allowed attempt(s).'}), 400
...
def submit_class_exam(course_id, exam_id):
    ...
    try:
        ...
        # Double-check attempt limit at submit time (guards against start-bypass)
        max_allowed = exam.max_attempts if (exam.max_attempts and exam.max_attempts > 0) else 1
        if attempts_used >= max_allowed:
            return jsonify({'error': f'Maximum attempts reached ({max_allowed}). Submission rejected.'}), 400

        # ---- Proctoring events (fail-soft: never block a submission) ----
        proctoring_events = []
        violation_count = 0
        try:
            raw_events = data.get('proctoring_events') or []
            if isinstance(raw_events, list):
                ALLOWED = {'tab_blur', 'tab_focus', 'fullscreen_exit', 'paste_detected',
                           'copy_detected', 'contextmenu_blocked', 'navigation_attempt'}
                VIOLATIONS = {'tab_blur', 'fullscreen_exit', 'paste_detected',
                              'copy_detected', 'navigation_attempt'}
                for ev in raw_events[:200]:  # cap to 200 events
                    if not isinstance(ev, dict):
                        continue
                    ev_type = str(ev.get('type', ''))[:40]
                    ...
                    if ev_type in VIOLATIONS:
                        violation_count += 1
        except Exception as _pe:
            print(f"[proctoring] event parse skipped: {_pe}")
