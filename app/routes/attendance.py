...
def attendance_enabled_required(f):
    """Decorator to check if attendance tracking is enabled in site settings"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        try:
            from app.models import SiteSettings
            settings = SiteSettings.query.first()
            if settings and hasattr(settings, 'attendance_tracking_enabled') and not settings.attendance_tracking_enabled:
                return jsonify({'error': 'Attendance tracking is disabled by administrator'}), 403
        except Exception:
            pass
        return f(*args, **kwargs)
    return decorated_function
