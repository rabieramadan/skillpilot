...
def check_auth():
    ...
    return jsonify({
        ...
        'is_logged_in': session.get('logged_in', False) or session.get('user_id') is not None,
        ...
        if user_obj:
            try:
                ...
                if 'phone' in data and hasattr(user_obj, 'phone'):
                    user_obj.phone = data['phone'].strip()
                if 'language' in data and hasattr(user_obj, 'language'):
                    user_obj.language = data['language'].strip()
...
def forgot_password_admin_reset():
    """Reset password for database users (admin creates reset token)"""
    data = request.json or {}
    username = data.get('username', '').strip()
    ...
    try:
        ...
        return jsonify({
            ...
            'token': token,
            'expires_in': '1 hour',
            'note': 'Email delivery not configured. Please share this token securely with the user.'
        })
    except Exception as e:
