...
class AuthManager {
    ...
    updateUIForLoggedInUser() {
        ...
        // Generic top-bar Attendance button intentionally hidden — attendance is
        // taken per class from the course/class detail view, not globally.
        const _att = document.getElementById('recordAttendanceBtn');
        if (_att) _att.style.display = 'none';
        document.getElementById('userLogoutBtn').style.display = '';
        const nw = document.getElementById('notifWrap');
        if (nw) nw.style.display = '';
        if (window.SkillPilotNotifications) window.SkillPilotNotifications.start();
    }
    ...
    updateUIForLoggedOutUser() {
        ...
        if (window.SkillPilotNotifications) window.SkillPilotNotifications.stop();
    }
