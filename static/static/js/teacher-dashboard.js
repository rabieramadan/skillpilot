class TeacherDashboard {
                            ...
                            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                <div style="flex: 1;">
                                    ...
                                    ${q.options && q.options.length > 0 ? `
                                        <div style="margin-top: 12px;">
                                            ${q.options.map((opt, i) => `
                                                <div style="display: flex; align-items: center; gap: 8px; padding: 6px 12px; background: ${q.correct_answer === i || (Array.isArray(q.correct_answer) && q.correct_answer.includes(i)) ? '#dcfce7' : 'white'}; border: 1px solid ${q.correct_answer === i || (Array.isArray(q.correct_answer) && q.correct_answer.includes(i)) ? '#16a34a' : '#e5e7eb'}; border-radius: 6px; margin-top: 4px;">
                                                    <i class="fas ${q.correct_answer === i || (Array.isArray(q.correct_answer) && q.correct_answer.includes(i)) ? 'fa-check-circle' : 'fa-circle'}" style="color: ${q.correct_answer === i || (Array.isArray(q.correct_answer) && q.correct_answer.includes(i)) ? '#16a34a' : '#d1d5db'};"></i>
                                ...
                                <div style="display: flex; gap: 8px;">
                                    <button onclick="teacherDashboard.deleteQuestion('${examId}', '${q.id}')" class="btn-secondary" style="padding: 6px 10px; color: #ef4444;" data-testid="button-delete-question-${q.id}">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
                            </div>
    ...
    toggleOptionsField() {
        ...
        if (type === 'short_answer') {
            ...
            select.innerHTML = '<option value="">-- Select correct answer --</option><option value="True">True</option><option value="False">False</option>';
        } else {
    ...
    updateExamCorrectAnswerDropdown() {
        ...
        if (options.length === 0) {
            ...
            select.innerHTML = '<option value="">-- Select correct answer --</option>' + 
                options.map(opt => `<option value="${this.escapeHtml(opt.trim())}">${this.escapeHtml(opt.trim())}</option>`).join('');
        }
    }
    ...
    async addQuestion(examId) {
        ...
        let options = [];
        let correctAnswer = document.getElementById('newQuestionAnswer').value;
        
        if (questionType === 'multiple_choice') {
            options = document.getElementById('newQuestionOptions').value.split('\n').filter(o => o.trim());
            // correctAnswer is already the actual text from dropdown
        } else if (questionType === 'true_false') {
            options = ['True', 'False'];
            // correctAnswer is "True" or "False" from dropdown
        }
        
        if (!correctAnswer && questionType !== 'short_answer') {
                ...
                <form id="aiGenerateForm" enctype="multipart/form-data">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                        <div>
                            ...
                            <input type="number" id="aiTrueFalseCount" min="0" max="20" value="5" 
                        ...
                        <div>
                            ...
                            <input type="number" id="aiMCQCount" min="0" max="20" value="5" 
                                   style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;" 
    ...
    async generateAIQuestions(examId) {
        ...
        try {
            ...
            try {
                ...
                throw new Error('Server returned an invalid response. Please check if you are logged in as a teacher.');
            }
                        ...
                        <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-bottom: 12px;">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                        ...
                                        <div style="margin-top: 12px; display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                                            <div>
                                                ${q.options.map((opt, optIdx) => `
                                                    <div style="padding: 6px 12px; background: white; border: 1px solid #e5e7eb; border-radius: 6px; margin-top: 4px; font-size: 14px;">
                                                        ${this.escapeHtml(opt)}
                                                    </div>
                                                `).join('')}
                                            </div>
                                            ${q.options_ar && q.options_ar.length > 0 ? `
                                ...
                                <div style="display: flex; flex-direction: column; gap: 8px;">
                                    <button onclick="teacherDashboard.showEditSurveyQuestionForm('${surveyId}', '${q.id}', ${JSON.stringify(q).replace(/"/g, '&quot;')})" class="btn-secondary" style="padding: 6px 10px; color: #0ea5e9;">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button onclick="teacherDashboard.deleteSurveyQuestion('${surveyId}', '${q.id}')" class="btn-secondary" style="padding: 6px 10px; color: #ef4444;">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
                            </div>
                ...
                <form id="editSurveyQuestionForm">
                    ...
                    <div id="editSurveyCorrectAnswerContainer" style="margin-bottom: 16px;">
                        ...
                        <select id="editSurveyQuestionCorrectAnswer" required style="width: 100%; padding: 10px; border: 1px solid #d1d5db; border-radius: 8px;">
                            ${isTrueFalse ? 
                                `<option value="">-- Select correct answer --</option>
                                 <option value="0" ${correctAnswer === 0 ? 'selected' : ''}>True</option>
                                 <option value="1" ${correctAnswer === 1 ? 'selected' : ''}>False</option>` :
                                `<option value="">-- Select correct answer --</option>
                                 ${options.map((opt, idx) => `<option value="${idx}" ${correctAnswer === idx ? 'selected' : ''}>${this.escapeHtml(opt)}</option>`).join('')}`
                            }
                        </select>
                    </div>
                    <div style="display: flex; gap: 12px; justify-content: flex-end;">
